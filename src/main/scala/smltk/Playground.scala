...
object Playground extends App {
  ...
  // var X = DenseMatrix.rand[Double](2000, 10)
  // val y = DenseVector((for(i <- 0 until 2000) yield scala.util.Random.nextInt(2)):_*)
  // val y = DenseVector.rand[Double](200)

  val X = loadIris()
  var y = DenseVector((for (i <- 0 until 150) yield { if(i<50) 1 else if(i<100) 2 else 3 }):_* )
  val neigh = new KNeighboursClassifier(nNeighbours=20)
  neigh.fit(X, y)
  println(neigh.predict(X(0 until 150, ::)))
  // println(neigh.predict(X))
